...
function analyzeSalesData(data, options) { // берем функ и кладем в нее нашу дату и опц (ее далее распакуем)
    ...
    data.purchase_records.forEach(receipt => { //в массиве чеков смотрим каждый чек
        ...
        receipt.items.forEach(item => { // смотрим каждый товар в чеке
            ...
            currentSeller.revenue += +(itemRevenue.toFixed(2)); //записывем каждую продажу конкретному продавцу по совпадаюзему чеку
            ...
            currentSeller.profit += +(itemProfit.toFixed(2)); // записываем продавцу прибыль где прибыль это вырукчка минус закупка
    ...
    const finalReport = sellersArray.map((seller, index) => { //будет новый массив на основе селлерс аррэй
        ...
        const calculatedBonus = calculateBonus(index, totalSellers, seller); // считаем бонус по функции для него вверху и нужен индекс чтобы распределить их места по прибыли
        ...
        return { // возвращаем вот такой объект (см заготовку выше)
            ...
            bonus: +(calculatedBonus.toFixed(2)) // бонус по функции рассчета (нужны все продавцы, индекс каждого и конкретный продавец из массива)
        };
    });
